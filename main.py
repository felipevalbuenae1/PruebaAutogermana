...
app.add_middleware(
    ...
    allow_origins=[
        "*"
    ],  # Permitir todos los dominios (puedes especificar uno en particular)
...
async def login(user: UserCreate, db=Depends(get_db)):
    db_user = await db.fetchrow("SELECT * FROM usuarios WHERE username = $1",
                                user.username)  # Buscar usuario en la BD
...
async def protected_route(user=Depends(verify_jwt)):
    return {
        "message": f"Hola, {user['sub']}, tienes acceso autorizado"
    }  # Mensaje de bienvenida si el token es válido
...
async def get_vehiculos(db=Depends(get_db), user=Depends(verify_jwt)):
    return await db.fetch("SELECT * FROM vehiculos"
                          )  # Consultar todos los vehículos en la BD
...
@app.put("/clientes/{id}")
async def update_cliente(id: int,
...
async def create_concesionario(concesionario: ConcesionarioCreate,
                               db=Depends(get_db),
                               user=Depends(verify_jwt)):
    await db.execute(
...
async def update_concesionario(id: int,
...
async def update_transaccion(id: int,
